...
// wasmVM: WASI mode_t and permission constants (mirrors POSIX values)
#[cfg(target_os = "wasi")]
#[allow(non_camel_case_types)]
pub type mode_t = u32;

#[cfg(target_os = "wasi")]
pub mod wasi_mode_consts {
...
impl FileInformation {
    ...
    /// Get information from a currently open file (WASI stub)
    #[cfg(target_os = "wasi")]
    pub fn from_file(_file: &impl AsFd) -> IOResult<Self> {
        Err(Error::new(ErrorKind::Unsupported, "from_file not yet supported on WASI"))
    }
...
// wasmVM: WASI version of display_permissions_unix (same logic, uses our constants)
#[cfg(target_os = "wasi")]
pub fn display_permissions_unix(mode: mode_t, display_file_type: bool) -> String {
    let mut result;
    if display_file_type {
...
// wasmVM: WASI paths are UTF-8 strings, check for trailing '/'
#[cfg(target_os = "wasi")]
pub fn path_ends_with_terminator(path: &Path) -> bool {
    path.as_os_str()
        .as_encoded_bytes()
